...
metadata {
	definition (name: "Hue Dimmer", namespace: "b-pass", author: "Bryan Pass, Scott G") {
		capability "Actuator"
...
def parse(String description) {
    ...
    // Tell st how many buttons we have
    if (getDataValue("numButtons") != 4) {
        response << createEvent(name: "numButtons", value: 4, displayed: false)
        response << createEvent(name: "numberOfButtons", value: 4, displayed: false)
    }
    ...
    if (msg.clusterId == 6) {
        ...
        //response << createEvent(name: "switch", value: (button == 1 ? "on" : "off"), isStateChange: true)
        response << createEvent(name: "button", value: "pushed", data: [buttonNumber: button], descriptionText: "$device.displayName button $button was pushed", isStateChange: true)
        log.debug "Pushed (power) $button $theSwitch"
    }
